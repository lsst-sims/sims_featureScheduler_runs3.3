...
class UexptDetailer(detailers.BaseDetailer):
    def __init__(self, exptime=30., filtername='u'):
...
def generate_blobs(
    ...
    for filtername, filtername2 in zip(filter1s, filter2s):
        ...
        if u_nexp1:
            detailer_list.append(detailers.FilterNexp(filtername="u", nexp=1))
        detailer_list.append(UexptDetailer(filtername="u", exptime=u_exptime))
        surveys.append(
            BlobSurvey(
                basis_functions,
                weights,
...
def ddf_surveys(detailers=None, season_unobs_frac=0.2, euclid_detailers=None, exptime=30.):
    obs_array = generate_ddf_scheduled_obs(season_unobs_frac=season_unobs_frac)

    # clobber the exposure time here
    obs_array['exptime'] = exptime
...
def main(args):
    ...
    u_exptime = args.u_exp
    other_exp = args.other_exp
    ...
    if dbroot is None:
        ...
        fileroot = dbroot + "_"
    file_end = "%i_other%i_v3.3_" % (u_exptime, other_exp)
    ...
    details = [
        ...
        UexptDetailer(exptime=u_exptime),
    ]
    euclid_detailers = [
        detailers.CameraRotDetailer(
            min_rot=-camera_ddf_rot_limit, max_rot=camera_ddf_rot_limit
...
if __name__ == "__main__":
    ...
    parser.add_argument("--u_exp", type=float, default=30)
    parser.add_argument("--other_exp", type=float, default=30)
