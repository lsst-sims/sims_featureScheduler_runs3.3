...
def _get_sun_ra_start(mjd_start):
    # likely a very expensive way to do this but OK.
    observatory = ModelObservatory(nside=32, mjd_start=mjd_start)
    conditions = observatory.return_conditions()
    return conditions.sun_ra_start


def make_rolling_footprints_shift_start(
    fp_hp=None,
    mjd_start=60218.0,
    nside=32,
    wfd_indx=None,
    ...
    scenario : str or None
        The scenario to run. If None, uses the default scenario of no shifts
        in the sequences of rolling cadences.
    ...
    all_slopes = [
        ...
        null_start + null_roll + np.roll(rolling, 0).tolist() + null_roll + null_end,  # noqa: E501
        null_start + null_roll + np.roll(rolling, 1).tolist() + null_roll + null_end,  # noqa: E501

        null_start + null_roll + null_roll + np.roll(rolling, 0).tolist() + null_end,  # noqa: E501
        null_start + null_roll + null_roll + np.roll(rolling, 1).tolist() + null_end,  # noqa: E501
    ]

    year_in_days = 365.25
    if scenario in ["sh1_0_sh2_0_sh3_0"]:
        shift_list = [0.0] * n_cycles
    elif scenario in ["sh1_0_sh2_0.5_sh3_1"]:
        shift_list = [
            0.0,
            0.5 * year_in_days,
            1.0 * year_in_days,
        ]
    elif scenario in ["sh1_-0.25_sh2_0.5_sh3_1"]:
        shift_list = [
            -0.25 * year_in_days,
            0.5 * year_in_days,
            1.0 * year_in_days,
        ]
    elif scenario in ["sh1_-0.25_sh2_0.25_sh3_0.75"]:
        shift_list = [
            -0.25 * year_in_days,
            0.25 * year_in_days,
            0.75 * year_in_days,
    ...
    #    print("    rolling sequence: %s" % " ".join(pmapping[_v] for _v in all_slopes[i]))  # noqa: E501

    fp_non_wfd = Footprint(
        mjd_start,
        sun_ra_start=_get_sun_ra_start(mjd_start),
        nside=nside,
    )
    rolling_footprints = []
    for i in range(len(all_slopes)):
        step_func = StepSlopes(rise=all_slopes[i])
        sun_ra_start = _get_sun_ra_start(mjd_start + all_shifts[i])
        rolling_footprints.append(
            Footprint(
                mjd_start + all_shifts[i],
    ...
    for key in hp_footprints:
        ...
        for i in range(len(rolling_footprints)):
            # make a copy of the current filter
            temp = hp_footprints[key] + 0
            # Set the non-rolling area to zero
            temp[non_wfd_indx] = 0

            # the i % 2 here maps back to the slices
            indx = split_wfd_indices[i % nslice]
            # invert the indices
            ze = temp * 0
            ze[indx] = 1
            temp = temp * ze
...
def main(args):
    ...
    footprints = make_rolling_footprints_shift_start(
        fp_hp=footprints_hp,
